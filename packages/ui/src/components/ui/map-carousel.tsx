...
const HotelCard = ({
  ...
  >
    ...
    {location.rating && (
      <div className="absolute top-2 right-2 bg-primary text-primary-foreground text-xs font-bold rounded-md px-1.5 py-0.5">
        {location.rating}
      </div>
    )}
    ...
    <div className="flex-1 min-w-0">
      ...
      {location.rating && (
        <div className="flex items-center gap-1 mt-1">
          <span className="bg-primary text-primary-foreground text-xs font-bold rounded-md px-1.5 py-0.5">
            {location.rating}
          </span>
        </div>
      )}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-1">
          {filterConfigs.map((config) => (
            <FilterSection
              ...
              onChange={(values) =>
                onFiltersChange({ ...filterState, [config.id]: values })
              }
            />
          ))}
        </div>
...
const VanillaLeafletMap = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { map: createLeafletMap } = L;
      const map = createLeafletMap(containerRef.current, {
        center,
            ...
            {filteredLocations.length === 0 ? (
              ...
              filteredLocations.map((location, index) => (
                <div
                  ...
                  ref={(el) => {
                    if (el) {
                      locationItemRefs.current.set(index, el);
                    }
                  }}
                >
                  <LocationListCard
