          ...
          <div className="space-y-2 pb-4">
            ...
            {hasMore && (
              ...
              >
                {showAll
                  ? "Show less"
                  : `View ${options.length - showLimit} more`}
              </button>
            )}
          </div>
        </div>
...
const DEFAULT_EVENTS: Event[] = [
  {
    ...
    image: "https://picsum.photos/seed/event-list-1/800/450",
  ...
  },
  {
    ...
    eventSignal: "popular",
    image: "https://picsum.photos/seed/event-list-2/800/450",
  ...
  },
  {
    ...
    image: "https://picsum.photos/seed/event-list-3/800/450",
  ...
  },
  {
    ...
    image: "https://picsum.photos/seed/event-list-4/800/450",
    ...
    (eventsToFilter: Event[], filtersToApply: FilterState): Event[] =>
      eventsToFilter.filter((event) => {
        if (
          ...
          (!event.category ||
            !filtersToApply.categories.includes(event.category))
        ) {
          return false;
        }
        if (filtersToApply.dates.length > 0) {
          ...
          const dateMatch = filtersToApply.dates.some((dateOption) => {
            if (dateOption === "Today") {
              return (
                dateTimeLower.includes("today") ||
                dateTimeLower.includes("tonight")
              );
            }
            ...
            if (dateOption === "This weekend") {
              return (
                dateTimeLower.includes("saturday") ||
                dateTimeLower.includes("sunday")
              );
            }
            if (dateOption === "This week") {
              ...
              if (dayNames.some((day) => dateTimeLower.includes(day))) {
                return true;
              }
              if (
                dateTimeLower.includes("today") ||
                dateTimeLower.includes("tonight") ||
                dateTimeLower.includes("tomorrow")
              ) {
                return true;
              }
              const inDaysMatch = dateTimeLower.match(/in (\d+) day/);
              if (inDaysMatch && Number.parseInt(inDaysMatch[1], 10) <= 7) {
                return true;
              }
              return false;
            }
        ...
        if (filtersToApply.neighborhoods.length > 0) {
          ...
          if (
            !filtersToApply.neighborhoods.some((n) =>
        ...
        if (filtersToApply.prices.length > 0) {
          const priceMatch = filtersToApply.prices.some((priceRange) => {
            ...
            if (priceRange === "$25 - $50") {
              return priceNum >= 25 && priceNum <= 50;
            }
            ...
            {filteredEvents.length === 0 ? (
              ...
              filteredEvents.map((event, index) => (
                <button
                  ...
                  ref={(el) => {
                    if (el) {
                      eventItemRefs.current.set(index, el);
                    }
                  }}
                  className={cn(
